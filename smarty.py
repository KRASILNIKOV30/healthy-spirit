...
def parse_arguments():
    ...
    parser.add_argument("--meta", action='store',
                        help="json with params, e.g.  --meta '{\"space\":\"1\", \"images\":[{\"name\": \"1\"}]}'")
...
def send(url, data, headers, timeout, files=None):
    ...
    if r.status_code == 200:
        ...
        print('[CLIENT_ERROR] request failed, status code: {}, response body: {} [url: {}]'.format(r.status_code,
                                                                                                   err_body, url))
    else:
        print(
            '[ERROR] request failed, status code: {}, response body: {} [url: {}]'.format(r.status_code, err_body, url))
